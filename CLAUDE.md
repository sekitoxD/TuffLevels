...
- **Step schema** (see the header comment in `Routes/Durotar.lua` for the authoritative reference): `type` is one of `accept | turnin | complete | grind | level | section | trainer | death | manual | travel | hearth | note`. Auto-detected types (`accept`, `turnin`, `complete`, `grind`, `level`) are checked in `IsStepDone()` in `Core.lua`; everything else requires the user to click Next. Steps optionally filter by `races`, `class`, `minLevel` so one file can serve multiple race/class combos. `section` steps are headers only (`Core:CurrentSection`/`Core:Sections` scan for them) and always report done.
