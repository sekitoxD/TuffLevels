...
| 16 | Data (+ Core): P2.4, P2.8 (reduced), Phase 3 `C_Map` guard | done (reviewed, one round of fixes applied: SetWaypoint's target-changed cache was left stale after a failed re-attempt following a TomTom waypoint removal, permanently blocking future attempts for that same target - fixed by invalidating the cache before any real attempt, not just on success). Core.lua wasn't touched (the dedup lives entirely in Data.lua). [G] pending: a route with two flightpath steps close together on the same map, confirm no visible delay learning a new flight point; advance/back through several steps and confirm no TomTom waypoint pile-up (check TomTom's own waypoint list) |
| 17 | UI: P2.6; Recorder: P2.9 + P2.10; Zones: P2.12 | done (reviewed across 2 rounds; the first hard-cap fix dropped the very entry that crossed the cap and pointed the player at a "start a fresh recording" fix that doesn't actually clear the log - both corrected: the cap check now runs after the insert, and the message points at the real `/tuff rec clear` command; `Start()`/`Restore()` also guard against resuming an already-full log). Known low-severity gap, not fixed: a zone change right at the cap can let the log overshoot by one entry (a section header isn't itself cap-checked) - nothing is lost, just slightly over 10000. [G] pending: toggle the tracker hidden/shown and confirm it catches back up instead of showing stale text; talk to an NPC with recording off and confirm no capture happens; an Alliance character opening Zones sees the "not authored" message, not Horde zones |
